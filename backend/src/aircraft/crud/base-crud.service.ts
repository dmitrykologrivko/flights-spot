...
export abstract class BaseCrudService<E extends IdentifiableObject, D extends IdentifiableObject> {
    ...
    async list<T extends ListQuery, U extends BasePaginatedResponse<D>>(
        input: T
        ...
        if (chain.hasPagination()) {
            output = await chain.mapPaginatedResponse(response => ({
                ...response,
    ...
    protected getPagination<T extends ListQuery>(
        input: T,
    ): (qb: SelectQueryBuilder<E>) => BasePagination<E, BasePaginatedResponse<E>> {
        return (qb) => null;
    }
